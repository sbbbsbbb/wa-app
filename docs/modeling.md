...
- 原生注册 HTTP 参数覆盖实际 App 主干形态：`/v2/exist`、`/v2/code`、`/v2/register` 统一复用稳定 profile/key bundle，发送 code/register 默认设备 map，并保留 SMS、voice、WA old、email OTP、passkey、silent auth、recaptcha 等投递/挑战方式的协议枚举与等待状态解析。
