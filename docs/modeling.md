...
- 原生注册 HTTP 参数覆盖实际 App 主干形态：`/v2/exist`、`/v2/code`、`/v2/register` 统一复用稳定 profile/key bundle，发送 code/register 默认设备 map，并保留 SMS、voice、flash、WA old、email OTP、send SMS、passkey、silent auth、recaptcha 等协议 taxonomy。真实 UI 可见 fallback 不是 taxonomy 全量：先按 `pref_reg_methods_order`（默认 `flash,sms,voice`）排序，再删除 wait 缺失或 `-1` 的 method，与服务端 `fallback_methods` 求交集，并按本地 eligibility/capability 过滤。冷却时间只按 APK 实际 wait 字段解析：`sms_wait`、`voice_wait`、`flash_wait`、`wa_old_wait`、`email_otp_wait`、`send_sms_wait`、`silent_auth_wait` 及 XMPP `*_wait_time`；`retry_after` 只作为当前请求的重试等待，不扩展为所有 fallback 可见项。
